...
## step 1
### あきらめたコード ### 
一回目の発想としては、まず2つのLinkedListを走査しながら、`node × 10 + 次のnode` のような形で数値へ変換し、  
その後2つの値を加算することを考えました。  
...
原因：  
1.数値に変換する処理が必要になり、実装が回りくどくなる。  
2.Python では問題ないが、他の言語では整数の桁数制限によりオーバーフローする可能性がある。  
